...
// Containers for all the containers methods
...
handlers._tokens.post = function(data, callback) {
  ...
  var password =
    ...
    data.payload.password.trim().length > 0
      ...
      : false;
  if (phone && password) {
    // Loookup the user that matches the phine number
    _data.read("users", phone, function(err, userData) {
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          callback(400, {
            Error: "Password did not match the specified user's stored passowrd"
          });
        }
      } else {
